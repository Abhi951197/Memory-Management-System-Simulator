...
function Heap() {
    ...
    // Allocate process to memory using worst fit method
    this.requestAllocation = function(process) {
        let blockWorstFit = null;
        let maxSpaceLeftover = -Infinity;
        ...
        while (block != null) {
            if (block.available && block.size >= process.size) {
                ...
                if (spaceLeftover > maxSpaceLeftover) {
                    blockWorstFit = block;
                    maxSpaceLeftover = spaceLeftover;
                }
            }
            block = block.next;
        }
        ...
        if (blockWorstFit != null) {
            ...
            if (maxSpaceLeftover > memControlBlockSize) {
                // Partition block if needed
                let newBlock = new MemControlBlock(maxSpaceLeftover);
                newBlock.next = blockWorstFit.next;
                newBlock.prev = blockWorstFit;
                if (blockWorstFit.next !== null) {
                    blockWorstFit.next.prev = newBlock;
                }
